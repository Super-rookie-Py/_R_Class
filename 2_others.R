...
data <- c(1:8)
data
m1 <- matrix(data=data, nrow=2, ncol=4) ; m1
...
# 데이터 파일로부터 데이터 프레임 생성

df_ch <- read.csv("C:/Users/거누/Desktop/Rstat/_R_Class/day01/data/character-deaths.csv")
df_ch
head(df_ch)
class(df_ch)
str(df_ch)

df_car <- read.table("data/cars.txt",
                     header=TRUE)
df_car
str(df_car)

df_cars <- read.csv("data/cars.csv", header = T)
head(df_cars)
str(df_cars)
...
fr2 <- read.table("data/fruits.txt" , nrows = 3) # 주석제외 
fr2
